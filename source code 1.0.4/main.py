...
class Button(turtle.Turtle):
    def __init__(self, image, id, x, y, image_size, on_screen, on_id, off_id):
        ...
        self.penup()
        screen.register_shape(f"{replit}assets/{resolution}/buttons/{image}.gif")
        self.shape(f"{replit}assets/{resolution}/buttons/{image}.gif")
    ...
    def checkif_clicked(self, x, y):
        ...
        if self.isvisible():
            if x >= self.area["left_bottom"][0] and x <= self.area["right_top"][0] and y >= self.area["left_bottom"][1] and y <= self.area["right_top"][1] and time.time() - click_delay > 0.1:
                clicked = self.id
                # Play a sound
                winsound.PlaySound(f"{replit}assets/sounds/click.wav", winsound.SND_ASYNC)
                ...
                if clicked == "new":
                    ...
                    for i in board:
                        if i.id[1] == -1:
                            ...
                            i.shape(f"{replit}assets/{resolution}/pieces/empty.gif")
...
class Board_Piece(turtle.Turtle):
    def __init__(self, image, id, x, y, distance, on_screen, on_id, off_id):
        turtle.Turtle.__init__(self)
        self.penup()
        screen.register_shape(f"{replit}assets/{resolution}/pieces/{image}.gif")
        self.shape(f"{replit}assets/{resolution}/pieces/{image}.gif")
        ...
        if self.isvisible():
            if x >= self.area["left_bottom"][0] and x <= self.area["right_top"][0] and y >= self.area["left_bottom"][1] and y <= self.area["right_top"][1] and time.time() - click_delay > 0.1:
                ...
                if not color_selected == "" and row == self.id[1]:
                    screen.register_shape(f"{replit}assets/{resolution}/pieces/{color_selected}.gif")
                    self.shape(f"{replit}assets/{resolution}/pieces/{color_selected}.gif")
    ...
    def checkif_filled(self):
        if row == self.id[1]:
            if self.shape() == f"{replit}assets/{resolution}/pieces/empty.gif":
                return 0
            else:
                return 1
        else:
...
class Color(turtle.Turtle):
    def __init__(self, image, id, x, y, distance, on_screen, on_id, off_id):
        ...
        self.image = f"{replit}assets/{resolution}/colors/{image}.gif"
        self.image2 = f"{replit}assets/{resolution}/colors/{image}_selected.gif"
...
class Guess_Info(turtle.Turtle):
    def __init__(self, x, y, image, on_id, off_id):
        turtle.Turtle.__init__(self)
        self.penup()
        screen.register_shape(f"{replit}assets/{resolution}/piece data/{image}.gif")
        self.shape(f"{replit}assets/{resolution}/piece data/{image}.gif")
        self.goto(x, y)
...
class Custom_Code(turtle.Turtle):
    def __init__(self, x, y, id, image, distance, on_screen, on_id, off_id):
        turtle.Turtle.__init__(self)
        self.penup()
        screen.register_shape(f"{replit}assets/{resolution}/code/{image}.gif")
        self.shape(f"{replit}assets/{resolution}/code/{image}.gif")
    ...
    def checkif_clicked(self, x, y):
        ...
        if self.isvisible():
            if x >= self.area["left_bottom"][0] and x <= self.area["right_top"][0] and y >= self.area["left_bottom"][1] and y <= self.area["right_top"][1] and time.time() - click_delay > 0.1:
                if not color_selected == "":
                    screen.register_shape(f"{replit}assets/{resolution}/code/{color_selected}.gif")
                    self.shape(f"{replit}assets/{resolution}/code/{color_selected}.gif")
                    self.image = color_selected
...
class Arrow(turtle.Turtle):
    def __init__(self, y, id, distance, on_screen, on_id, off_id):
        turtle.Turtle.__init__(self)
        self.penup()
        screen.register_shape(f"{replit}assets/{resolution}/arrow.gif")
        self.shape(f"{replit}assets/{resolution}/arrow.gif")
...
class Text(turtle.Turtle):
    def __init__(self, image, id, x, y, on_screen, on_id, off_id):
        turtle.Turtle.__init__(self)
        self.penup()
        screen.register_shape(f"{replit}assets/{resolution}/text/{image}.gif")
        self.shape(f"{replit}assets/{resolution}/text/{image}.gif")
...
class Ending_Text(turtle.Turtle):
    def __init__(self, image, id, x, y, on_screen):
        turtle.Turtle.__init__(self)
        screen.register_shape(f"{replit}assets/{resolution}/text/{image}.gif")
        self.shape(f"{replit}assets/{resolution}/text/{image}.gif")
...
def check_code():
    ...
    if correct == 4:
        ...
        try:
            win = threading.Thread(target=you_win.flash)
            win.start()
        except:
            pass
        winsound.PlaySound(f"{replit}assets/sounds/yes.wav", winsound.SND_ASYNC)
    ...
    if row == 0:
        ...
        try:
            lose = threading.Thread(target=you_lose.flash)
            lose.start()
        except:
            pass
        winsound.PlaySound(f"{replit}assets/sounds/fail.wav", winsound.SND_ASYNC)

    # Sounds
    if row > 0:
        if correct == 0 and location == 0:
            winsound.PlaySound(f"{replit}assets/sounds/none.wav", winsound.SND_ASYNC)
        elif not correct == 4:
            winsound.PlaySound(f"{replit}assets/sounds/good.wav", winsound.SND_ASYNC)
...
# Text
texts.append(Text("code", 0, 0 * monitor_ratio[0], 300 * monitor_ratio[1], False, ["play", "new"], ["random", "back", "custom"]))
you_win = Ending_Text("youwin", "youwin", 0, 0, False)
you_lose = Ending_Text("youlose", "youlose", 0, 0, False)
